...
class SubscribeScore(object):
        ...
        if len(student) != 0:
            ...
            if len(subscribe_student) == 0:
                try:
                    ...
                    logger.error("[write subscribe student info repeated]: [openid]: %s [Exception]: %s" % (openid, e))
            else:
                if subscribe_student[0].status == 0:
                    models.SubscribeStudent.objects.filter(openid=openid).update(status=1)
                    res["message"] = "success"
                else:
                    res["message"] = "repeated"
        else:
    ...
    @classmethod
    def score_compare(cls, openid, sno, passwd, name, db_get_scores):
        """
                ...
                for i in range(len(courses)):
                    ...
                    if re.search(r"(\d+)", db_write_scores[i]):
                        if cur_score != float(db_write_scores[i]):
                            ...
                            if res_code == 43101:
                                # 当用户取消订阅的时候
                                models.SubscribeStudent.objects.filter(openid=openid).update(status=0)
                            else:
                                models.SubscribeStudent.objects.filter(openid=openid).update(scores=db_write_scores_str)
                            break
                    else:
            ...
            for subscribe_student in subscribe_students:
                try:
                    ...
                    cls.score_compare(subscribe_student.openid, cur_student.sno,
                                      ...
                                      cur_student.name, db_scores)
                except Exception as e:
                    logger.error("[遍历当前student失败][Exception]: %s" % e)
            time.sleep(5)
